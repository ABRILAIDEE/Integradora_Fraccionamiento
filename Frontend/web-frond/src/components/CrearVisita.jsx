...
const CrearVisita = () => {
  ...
  // ✅ Función para compartir el enlace dinámico
  const handleCompartirEnlace = async () => {
    const url = new URL(window.location.href);
    url.pathname = '/create-visit';

    url.searchParams.set('name', 'Juan Pérez');
    url.searchParams.set('numPersons', '5');
    url.searchParams.set('description', 'Vamos con el fin de saludar a una amiga');
    url.searchParams.set('vehiclePlate', 'OSK-128-9FKL');
    url.searchParams.set('houseNumber', '01');
    url.searchParams.set('visitDate', '07/12/10 00:00');
    url.searchParams.set('visitStatus', 'Pendiente');
    url.searchParams.set('keyword', 'Plátano');
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: 'Comparte este enlace para ver los detalles de la visita:',
          url: link,
        });
      } catch (error) {
  ...
  const handleGenerarQR = () => {
    const visitData = {
      name: 'Juan Pérez',
      numPersons: '5',
      description: 'Vamos con el fin de saludar a una amiga',
      vehiclePlate: 'OSK-128-9FKL',
      houseNumber: '01',
      visitDate: '07/12/10 00:00',
    ...
    <div style={styles.container}>
      ...
      <div style={styles.formContainer}>
        ...
        <div style={styles.buttonContainer}>
          <button onClick={handleCompartirEnlace} style={styles.button}>
            Compartir enlace
          </button>
          <button onClick={handleGenerarQR} style={styles.button}>
            Generar código QR
          </button>
        </div>
      </div>
